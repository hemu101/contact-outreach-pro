...
import { Loader2, Search, Globe, Linkedin, Users, ExternalLink, CheckCircle2, XCircle, Sparkles } from 'lucide-react';
...
export function LinkFinderPage() {
  ...
  const handleFind = async () => {
    ...
    try {
      ...
      setResult({ website, linkedin, verified: vData?.success && verification?.status === 'Proper', verification });
              ...
              {result.verification && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                  {result.verification.emails?.slice(0, 1).map((e: string) => (
                    <Badge key={e} variant="outline" className="justify-start truncate">📧 {e}</Badge>
                  ))}
